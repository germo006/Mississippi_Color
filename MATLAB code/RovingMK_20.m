...
function [S, Hval, pvals] = RovingMK_20(V, alpha)

Hval = zeros(length(V)-19,1);
pvals = zeros(length(V)-19,1);
S = zeros(length(V)-19,1);

for ii = 1:length(V)-19
    [Hvaliter,pvaliter] = Mann_Kendall(V(ii:ii+19,1),alpha);
    Hval(ii) = Hvaliter;
    pvals(ii) = pvaliter;
    if pvaliter < alpha
        [S(ii), ~] = TheilSen([[1:20]', V(ii:ii+19,1)]);
    else
        S(ii) = NaN;
    end
end
