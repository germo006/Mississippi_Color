...
function [S,Hval, pvals] = RovingMK_30(V, alpha)

Hval = zeros(length(V)-29,1);
pvals = zeros(length(V)-29,1);
S = zeros(length(V)-29,1);

for ii = 1:length(V)-29
    [Hvaliter,pvaliter] = Mann_Kendall(V(ii:ii+29,1),alpha);
    Hval(ii) = Hvaliter;
    pvals(ii) = pvaliter;
    if pvaliter < alpha
        [S(ii), ~] = TheilSen([[1:30]', V(ii:ii+29,1)]);
    else
        S(ii) = NaN;
    end
end
