...
function [S, Hval, pvals] = RovingMK_40(V, alpha)

Hval = zeros(length(V)-39,1);
pvals = zeros(length(V)-39,1);
S = zeros(length(V)-39,1);

for ii = 1:length(V)-39
    [Hvaliter,pvaliter] = Mann_Kendall(V(ii:ii+39,1),alpha);
    Hval(ii) = Hvaliter;
    pvals(ii) = pvaliter;
    if pvaliter < alpha
        [S(ii), ~] = TheilSen([[1:40]', V(ii:ii+39,1)]);
    else
        S(ii) = NaN;
    end
end
